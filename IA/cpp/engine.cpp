...
Tensor matmul(const Tensor& A, const Tensor& B) {
    ...
    // Cas 3D x 2D : (batch, M, K) x (K, N) -> (batch, M, N)  (numpy broadcast)
    // La matrice 2D B est reutilisee pour chaque element du batch.
    if (A.ndim() == 3 && B.ndim() == 2) {
        int64_t batch = A.shape[0];
        int64_t M = A.shape[1], K = A.shape[2];
        int64_t K2 = B.shape[0], N = B.shape[1];
        if (K != K2) throw std::runtime_error("matmul: shape mismatch 3Dx2D");
    ...
    // Cas 2D x 3D : (M, K) x (batch, K, N) -> (batch, M, N)  (numpy broadcast)
    if (A.ndim() == 2 && B.ndim() == 3) {
        int64_t batch = B.shape[0];
        int64_t M = A.shape[0], K = A.shape[1];
        int64_t K2 = B.shape[1], N = B.shape[2];
        if (K != K2) throw std::runtime_error("matmul: shape mismatch 2Dx3D");
        Tensor C(Shape({batch, M, N}));
        for (int64_t b = 0; b < batch; ++b) {
            matmul_2d(A.ptr(), M, K,
                      B.ptr() + b * K * N, K, N,
...
Tensor sum_axis(const Tensor& x, int axis) {
    ...
    if (axis == 0) {
        ...
        // Preserve les dimensions restantes (x.shape[1:]) au lieu d'aplatir
        // en 1D, pour correspondre au comportement numpy sum(axis=0).
        std::vector<int64_t> out_dims(x.shape.dims.begin() + 1, x.shape.dims.end());
